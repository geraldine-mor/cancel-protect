...
from utils.charts import hypothesis_bar_plot, hypothesis_2_plot, hypothesis_3_plot
...
with tab1:
    st.subheader("Hypothesis 1: We suspect that no deposit bookings cancel more " \
    "than deposit secured bookings")

    st.error("**Hypothesis Result: Rejected.** The relationship exists, but runs" \
    " opposite to what was proposed.")

    st.markdown(
        "A Chi-Square test confirmed a statistically significant relationship between "
        "`deposit_type` and cancellation (p < 0.001), with a moderate effect size "
        "(Cramer's V = 0.48)."
    )
    ...
    with col1:
        st.markdown(
            "However, the direction is inverted from what H1 proposed: rather than "
            "deposit-secured bookings showing lower cancellation risk, **Non Refund** "
            "bookings cancel at close to 100% - the highest rate of any group in the "
            "dataset."
        )

        st.markdown(
            "A cancellation rate this extreme suggests Non Refund is unlikely to reflect "
            "genuine guest commitment. It more plausibly reflects an internal booking or "
            "operational process - which is also why the feature was excluded from the "
            "predictive model (see Model Evaluation page) despite its strength as a "
            "descriptive signal."
        )
    ...
    with col2:
        st.dataframe(hypothesis_1_crosstab(df), 
                    column_config={
                        "not_canceled": st.column_config.NumberColumn(format="percent"),
                        "canceled": st.column_config.NumberColumn(format="percent"),
                    })
...
with tab2:
    st.subheader("Hypothesis 2: Bookings with longer lead times have a higher cancellation" \
    " rate than last-minute bookings")
    
    st.success("**Hypothesis Result: Accepted.** Longer range bookings cancel more than" \
